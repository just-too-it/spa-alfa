...
export const Home = () => {
  ...
  return (
    <main className={clsx('container', styles.home)}>
      ...
      {/* {status === STATUS.LOADING && <div className={styles.message}>Загрузка данных</div>} */}
      {status === STATUS.LOADING && <div className={styles.loader}><Loader/></div>}
      ...
      {status === STATUS.SUCCESS && (
        <>
          ...
          <h2 className={styles.title}>{showFavoriteBooks ? `Избранные книги: ${favoriteBooks.length} шт.` : 'Все книги'}</h2>
